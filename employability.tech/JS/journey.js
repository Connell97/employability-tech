...
const changeStep = (event) => {
    ...
    for (let i = 0; i < timelineDots.length; i++) {
        if (timelineDots[i].classList.contains('active')) {
            ...
            journeyImg.src = `IMG/${journeySteps[i][0]}`;
            journeyInfo.innerHTML = journeySteps[i][1];

        }
    }
}

prevStepBtn.addEventListener('click', changeStep);
